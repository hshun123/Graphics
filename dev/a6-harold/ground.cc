                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Point3 start, end;
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &start);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[stroke2d.size() - 1], &end);
  
    Vector3 plane_x = start - end;
    Vector3 normal = Vector3::Normalize(plane_x.Cross(Vector3(0, 1, 0)));
    normal.Dot(look) > 0 ? (normal = -normal) : normal;
    ...
    for (int i = 0; i < stroke2d.size() - 1; i++) {
        ...
        Point3 mouseIn3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix,
            stroke2d[i]);
        Vector3 dir = (mouseIn3d - eye).ToUnit();
        Ray eyeThroughMouse(eye, dir);

        float t;
        eyeThroughMouse.IntersectPlane(start, normal, &t, &cp);
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        float h = hfunc(normal, curve_points, P.ClosestPointOnPlane(start, normal));
        float d = P.DistanceToPlane(start, normal);
        float w = 1 - pow(d / 5.0, 2);
        if ((1 - pow(d / 5.0, 2)) <= 0) {
            w = 0;
        }
        if (h != 0) {
            P = Point3(P[0], (1 - w) * P[1] + w * h, P[2]); // equation from the paper
        }
