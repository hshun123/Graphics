                        ...
                        const Point2 &normalized_screen_pt, Point3 *sky_point)
{
    ...
    float t;
    return eyeThroughMouse.IntersectSphere(Point3(0, 0, 0), 1500.0, &t, sky_point);
}
                       ...
                       const Mesh &stroke2d_mesh, const Color &stroke_color)
{
    ...
    for (int i = 0; i < stroke2d_mesh.num_vertices(); i++) {
        ...
        Point3 p = m.vertexReadOnly(i);
        ScreenPtHitsSky(view_matrix, proj_matrix, Point2(stroke2d_mesh.vertexReadOnly(i).x(),
            stroke2d_mesh.vertexReadOnly(i).y()), &p);
