...
void QuakeApp::DrawUsingOpenGL() {
    ...
    if (earth_.Globe()) {
         model_matrix =
            Matrix4::RotationY(rot_angle); // rotate the earth sphere
    }
    ...
    // Eric
    Earthquake earthquake_marker;
    Point3 marker_pos;
    float marker_size;

    for (int i = 0; i < quake_db_.max_index(); i++) {
        // earthquake
        earthquake_marker = quake_db_.earthquake(i);

        // a * b - c where a, b, c correlate with sizes of the magnitude
        marker_size = (earthquake_marker.magnitude()/100) * 
            (earthquake_marker.magnitude()/ quake_db_.min_magnitude()) - (earthquake_marker.magnitude()/120); 
        
        // data within past one year of current time
        if (earthquake_marker.date().ToSeconds() <= current_time_ 
            && earthquake_marker.date().ToSeconds() >= current_time_ - (365 * 24 * 60 * 60)) { // year in sec: 365 * 24 * 60 * 60

            if (earth_.Globe()) {
                marker_pos = earth_.LatLongToSphere(earthquake_marker.latitude(), earthquake_marker.longitude());
            }
            else {
                marker_pos = earth_.LatLongToPlane(earthquake_marker.latitude(), earthquake_marker.longitude());
            }

            // normalization process for color of the marker
            // reference from worksheet
            // r-value
            float normalization_r = 
                (earthquake_marker.magnitude() - quake_db_.min_magnitude()) / (quake_db_.max_magnitude() - quake_db_.min_magnitude());
            // g-value
            float normalization_g = 1 - normalization_r;

            Matrix4 model_Earthquake = model_matrix *
                Matrix4::Translation(marker_pos - Point3(0, 0, 0)) *
                Matrix4::Scale(Vector3(marker_size, marker_size, marker_size));

            quick_shapes_.DrawSphere(model_Earthquake, view_matrix_, 
                proj_matrix_, Color(2 * normalization_r, normalization_g, normalization_g + 0.1)); // color of the sphere differ by normalization
        }
    }
}