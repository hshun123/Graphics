                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    Matrix4 ctm = parent_transform * skeleton_.RotAxesSpaceToBoneSpace(bone_name)
        * pose_.JointRotation(bone_name) * skeleton_.BoneSpaceToRotAxesSpace(bone_name);

    Vector3 bone = skeleton_.BoneDirectionAndLength(bone_name);
    Point3 zero = Point3(0.0, 0.0, 0.0);
    Point3 bone_point = Point3(bone.x(), bone.y(), bone.z());
    ...
    if (bone_name == "lfemur" || bone_name == "rfemur") {
        quick_shapes_.DrawLineSegment(ctm , view_matrix, proj_matrix, Color(0.0, 0.0, 0.0), zero, bone_point, 0.01);
    }
    ...
    if (bone_name == "lowerback") {
        Matrix4 low_S = Matrix4::Scale(Vector3(0.1, 0.2, 0.1));
        Matrix4 low_trans = Matrix4::Translation(-bone / 2);
        quick_shapes_.DrawSphere(ctm * low_trans * low_S, view_matrix, proj_matrix, Color(1, 0.0, 0.0));
    }
    if (bone_name == "upperback") {
        Matrix4 upper_S = Matrix4::Scale(Vector3(0.1, 0.1, 0.1));
        quick_shapes_.DrawSphere(ctm *  upper_S, view_matrix, proj_matrix, Color(1, 0.0, 0.0));
    }
    if (bone_name == "thorax") {
        Matrix4 thorax_S = Matrix4::Scale(Vector3(0.1, 0.1, 0.1));
        Matrix4 thorax_translate = Matrix4::Translation(4 * bone);
        quick_shapes_.DrawSphere(ctm * thorax_S * thorax_translate, view_matrix, proj_matrix, Color(1, 0.0, 0.0));
    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
        //
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.5, 0.0, 0.0), zero, zero + bone, 0.02);
    }
    if (bone_name == "head") {
        //
        Matrix4 head_S = Matrix4::Scale(Vector3(0.1, 0.2, 0.1));
        Matrix4 head_trans = Matrix4::Translation(Vector3(0.0, 0.0, 0.09));
        quick_shapes_.DrawSphere(ctm * head_trans * head_S, view_matrix, proj_matrix, Color(1, 0.0, 0.0));

        Point3 antenna = Point3(0.0, 0.3, 0.1);
        Matrix4 antenna_matrix = Matrix4::Translation(Vector3(0.0, 0.10, 0.0));
        Matrix4 rotate = Matrix4::RotationZ(0.25);
        quick_shapes_.DrawLineSegment(ctm *  rotate * antenna_matrix, view_matrix, proj_matrix, Color(0, 0, 0), zero, antenna, 0.005);
  
        rotate = Matrix4::RotationZ(-0.25);
        quick_shapes_.DrawLineSegment(ctm * rotate * antenna_matrix, view_matrix, proj_matrix, Color(0, 0, 0), zero, antenna, 0.005);
